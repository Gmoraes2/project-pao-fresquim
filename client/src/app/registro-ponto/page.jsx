...
export default function RegistroPontoPage() {
  ...
  useEffect(() => {
    async function carregarFuncionarios() {
      ...
      try {
        const lista = (await listarFuncionariosResumo()) || [];
        ...
        toast.error(getApiErrorMessage(error, "Erro ao buscar funcionarios."));
      } finally {
        setIsLoading(false);
      }
    }
  ...
  async function carregarHistorico(id) {
    ...
    try {
      const registros = await listarPontosPorFuncionario(id);
      setRegistros(registros || []);
    } catch (error) {
      toast.error(getApiErrorMessage(error, "Erro ao buscar historico de ponto."));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function registrarPonto(tipo) {
    ...
    try {
      await registrarPontoService(funcionarioId, tipo);
      await carregarHistorico(funcionarioId);
      toast.success("Ponto registrado com sucesso!");
    } catch (error) {
      toast.error(getApiErrorMessage(error, "Erro ao registrar ponto."));
    } finally {
      setIsSaving(false);
    }
  }
