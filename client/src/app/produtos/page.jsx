...
export default function ProdutosPage() {
  ...
  async function getProdutos(params) {
    setIsLoading(true);
    try {
      const produtos = await listarProdutos(params);
      setProdutos(produtos);
    } catch (error) {
      toast.error(getApiErrorMessage(error, "Erro ao buscar produtos."));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleSaveProduto = async (dadosDoFormulario) => {
    try {
      if (dadosDoFormulario.id) {
        await atualizarProduto(dadosDoFormulario.id, dadosDoFormulario);
        toast.success("Produto atualizado com sucesso!");
      } else {
        await criarProduto(dadosDoFormulario);
