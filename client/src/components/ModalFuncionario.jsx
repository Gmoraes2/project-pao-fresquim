...
export default function ModalFuncionario({
  ...
  const podeAcessarFerias = () => {
    if (!formData.dataAdmissao) return false;

    const admissao = new Date(`${formData.dataAdmissao}T00:00:00`);
    const hoje = new Date();
    const umAnoDepois = new Date(admissao);
    umAnoDepois.setFullYear(umAnoDepois.getFullYear() + 1);

    return hoje >= umAnoDepois;
  };

  const feriasDisponiveis = podeAcessarFerias();

  const explicarFerias =
    "Ferias ficam disponiveis somente apos 1 ano da data de admissao.";
  ...
  const alterarLicenca = (index, campo, valor) => {
    if (campo === "tipoLicenca" && valor === "Férias" && !feriasDisponiveis) {
      toast.error(explicarFerias);
      return;
    }
  ...
  const handleSubmit = (e) => {
    ...
    const possuiFerias =
      ...
      (formData.licencas || []).some(
        (licenca) => licenca.tipoLicenca === "Férias",
      );

    if (possuiFerias && !feriasDisponiveis) {
      toast.error(explicarFerias);
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              if (e.target.value === "Férias" && !feriasDisponiveis) {
                toast.error(explicarFerias);
                return;
              }
          ...
          >
            ...
            <option value="Férias" disabled={!feriasDisponiveis}>
              Férias
            </option>
          </select>
          {!feriasDisponiveis && (
