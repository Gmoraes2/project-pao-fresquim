...
import { listarFuncionariosResumo } from "@/services/funcionarioService";
...
export default function VendasPage() {
  ...
  async function carregarDados() {
    setIsLoading(true);
    try {
      const [produtos, clientes, funcionarios, vendas] = await Promise.all([
        listarProdutos(),
        listarClientes(),
      ...
      toast.error(getApiErrorMessage(error, "Erro ao carregar dados de venda."));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const finalizarVenda = async () => {
    ...
    if (!funcionarioId) {
      toast.error("Selecione o funcionario responsavel pela venda.");
      return;
    }
    ...
    try {
      ...
      const vendaCalculada = await calcularTotalVenda(payload);
      const notaFiscal = await gerarNotaFiscal({
        ...vendaCalculada,
        valorTotal: vendaCalculada.valorTotal,
      });
      ...
      toast.error(getApiErrorMessage(error, "Erro ao finalizar a venda."));
    } finally {
      setIsSaving(false);
    }
  };
                    ...
                    vendas
                      ...
                      .map((venda) => (
                        <tr key={venda.id}>
                          ...
                          <td className="px-6 py-4 text-gray-500">
                            {venda.funcionarioNome || "Nao informado"}
                          </td>
                          <td className="px-6 py-4 text-gray-500">
              ...
              >
                ...
                {funcionarios.map((funcionario) => (
                  <option key={funcionario.id} value={funcionario.id}>
                    {funcionario.nome} - {funcionario.cargo || "Equipe"}
                  </option>
                ))}
              </select>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          alterarQuantidade(item.id, item.quantidadeVenda - 1)
                        }
